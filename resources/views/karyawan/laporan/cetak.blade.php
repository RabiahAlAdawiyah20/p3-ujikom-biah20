...
<body>
    <div class="container-fluid">
        <table>
            <thead>
                <tr>
                    <th colspan="4">Laporan <strong>{{ $data->invoice }}</strong></th>
                    <th>{{ $data->created_at->format('d-M-Y') }}</th>
                </tr>
                <tr>
                    <td style="padding-bottom:80px" colspan="2">
                        <h3 style="color:coral">{{ $data->user->nama_cabang }}</h3>
                        <p class="text-muted m-l-5"> Diterima Oleh <span style="margin-left:8px"> </span>:
                            {{ $data->user->name }}
                    ...
                    <td colspan="3">
                        ...
                        <p style="text-align:right"><b>Tanggal Masuk :</b> <i class="fa fa-calendar"></i>
                            {{ Carbon\Carbon::parse($data->tgl_transaksi)->format('d-m-y') }}</p>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:black"><input type="hidden" value="{{ $disc = ($hitung * $item->disc) / 100 }}">
                        {{ Rupiah::getRupiah($disc) }}</td>
                </tr>
                <tr>
        ...
        <ol style="font-size: 12px">
            @foreach ($bank as $banks)
                <li style="color: black"> {{ $banks->nama_bank }} <br> {{ $banks->no_rekening }} a/n
                    {{ $banks->nama_pemilik }}</li>
            @endforeach
        </ol>
    </div>
</body>
